                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('item.store') }}" class="form-validate">
                        ...
                        <div class="form-group row required">
                            ...
                            <div class="col-md-6">
                                <input id="quantity" type="number" class="form-control{{ $errors->has('qty') ? ' is-invalid' : '' }}" name="qty" value="{{ old('qty') }}" required>
                            ...
                            <div class="col-md-6">
                                <select id="manufacturer" required class="form-control{{ $errors->has('manufacturer') ? ' is-invalid' : '' }}" name="manufacturer">
                                    ...
                                    @foreach(config('setting')['manufacturer'] as $key => $man)
                                        <option @if(old('manufacturer',0) == $key) selected="selected" @endif value="{{ $key }}">{{ $man }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6">
                                <select id="model" required class="form-control{{ $errors->has('model') ? ' is-invalid' : '' }}" name="model">
                                    ...
                                    @foreach(config('setting')['models'] as $keyModel => $model)
                                        <option @if(old('model',0) == $keyModel) selected="selected" @endif value="{{ $keyModel }}">{{ $model }}</option>
                                    @endforeach
                                </select>
                                @if ($errors->has('model'))
                                    <span class="invalid-feedback" role="alert">
                        ...
                        <div class="form-group row required">
                            ...
                            <div class="col-md-6">
                                <input id="date_of_purchased" required readonly="" type="text" class="form-control datepicker {{ $errors->has('date_of_purchased') ? ' is-invalid' : '' }}" name="date_of_purchased" value="{{ old('date_of_purchased',date('d-m-Y')) }}">
